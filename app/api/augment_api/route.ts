...
export async function POST(req: NextRequest) {
  ...
  return new NextResponse(searchResult)
}
...
export async function generateResearchQuestions(inputText : string) {
  ...
  if (run.status === 'completed') {
    ...
    let questionsList: string[] = Array.from(JSON.parse(questionsString).questions);
...
export async function generateAugmentedTextWithSearchResults(inputText : string, searchResult : string) {
  const thread = await openai.beta.threads.create();
  let prompt = "Input Essay: \n" + inputText + "\n\nSearch Result: " + searchResult;
